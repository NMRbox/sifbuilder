...
process_timestamp = datetime.now().strftime("%Y-%m-%d %H:%M")
            ...
            for line in f:
                ...
                try:
                    if not file_path.exists():
                        builder_logger.debug(f"{file_path} does not exists")
                        continue
                    if not os.access(file_path, os.X_OK):
                        builder_logger.debug(f"{file_path} is not executable")
                        continue
                ...
                if file_path.parent in path_dirs:
                    ...
                    builder_logger.debug(f"{file_path} parent not in path_dirs")
